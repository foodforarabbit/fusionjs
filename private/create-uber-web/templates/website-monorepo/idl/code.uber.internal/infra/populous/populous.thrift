...
struct FreightInfo {
  ...
  3: optional string mcOrDotNumber,
...
struct UpdateUserInfoRequest
{
   ...
   * Unless explicitely stated, Money services endpoints do not expect those
...
service UserService
{
  ...
  /**
   * Add/update user in the new elasticsearch cluster.
   * This endpoint should only be triggered by the new ingester that
   * listens to updates to UDR columns via schemaless triggers.
   * This index executes the new indexing implementation that
   * generates the ES document according to a new schema.
   */
  void indexUserInElasticSearch(
    1: required UUID userUuid,
  ) throws (
    1: EntityNotFound notFound,
    2: Unauthorized unauthorized,
    3: InternalServerError serverError,
    4: ValidationError validationFailed
    5: RateLimitedError rateLimitedError
  )

  /**
   * temporary endpoint to test ES backfill
   * Calling indexUserInElasticSearch in bulk
   */
  list<UUID> bulkIndexUserInElasticSearch(
    1: required list<UUID> userUuids,
  ) throws (
    1: EntityNotFound notFound,
    2: Unauthorized unauthorized,
    3: InternalServerError serverError,
  ...
  /** Backfill for ElasticSearch */
  void startBackfillElasticSearch(
    1: required string clusterUrl,
  ...
  /** Kill Backfill for ElasticSearch */
  void killBackfillElasticSearch(
    1: i32 timeout
  ) throws (
    1: Unauthorized unauthorized,
    2: ValidationError validationFailed,
