...
tape('Server Renderer with options true', async t => {
  ...
  t.ok(
    ctx.body.includes("<link rel='icon' type='image/x-icon' href='test.ico' />")
  );
  t.ok(ctx.body.includes("<div id='root'></div>"));
  t.end();
});

tape('Server Renderer with options false', async t => {
  const app = new App('test');
  app.register(RenderToken, ServerPlugin);
  app.register(PageSkeletonConfigToken, {
    includeFonts: false,
