...
import {
  S3ConfigToken,
  AssetProxyingResponseHeaderOverrides,
} from '../src/tokens';
import AssetProxyingPlugin from '../src/server.js';
...
test('Server Client', done => {
  (async () => {
    ...
    await upload({
      directory: path.join(process.cwd(), '__tests__/fixture-files'),
      s3Config,
    });
...
test('s3 config reading makes sense', () => {
  expect(() =>
    getConfig({secretsPath: `__tests__/fixture/inexistent.json`})
  ).toThrow(/Could not find/);
  expect(() =>
    getConfig({secretsPath: `__tests__/fixture/invalid_json.txt`})
  ).toThrow(/Invalid JSON/);
  expect(() =>
    getConfig({secretsPath: `__tests__/fixture/invalid_secrets.json`})
